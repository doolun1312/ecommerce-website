...
// This function is called after submitting the form.
function storeCustomers(message) {
  // If the server returns "ok", submit the form with jQuery.
  if (message == "ok") {
    $("#form1")[0].submit();
  } 
...
// This function binds the submit event to the form with ID "form1".
$("#form1").submit(function (e) {
  ...
  $.ajax({
    type: "POST", 
    data: {"name" : name , "surname" : surname , "username" : username , "email" : email ,"oldpassword" : oldpassword , "password" :  password ,  "password1" :  password1},
    url: "editinfovalidation.php", 
    success: function(result){
      // Call the storeCustomers function with the server's response.
